...
def testMe(
    fixXi=0.02617993877991494365,
    LiType="Sparse",
    RossType="Thick",
    file="polder.modis.tiles.cover.04.dat.count.top.1.all.h08v05.256.509.dat",
    ofile=False,
    type=1,
    N=1000,
):
    ...
    rmse, xi, P, FWD, x, y, phi = lutInvertRossHS(
        VZA, SZA, RAA, REFL, LiType=LiType, RossType=RossType, N=N, fixXi=fixXi
    )
    if ofile == True:
        ...
        FILE.write(
            ...
            + " rmse = "
            + str(rmse)
            + "1:vza 2:sza 3:relphi 4:obs(443) 5:mod(443) 6:obs(565) 7:mod(565) 8:obs(670) 9:mod(670) 10:obs(765) 11:mod(765) 12:obs(865) 13:mod(865)\n"
        )
        for i in range(len(VZA)):
            ...
            FILE.write(ostr)
        FILE.close()
    vza = numpy.array(list(range(141))) * 1.0 - 70
    raa = vza * 0.0
    ...
    if ofile == True:
        ...
        FILE = open(aofile, "w")
        FILE.write(
            "# pplane plot at mean sza of observations: sza = " + str(sza[0]) + "\n"
        )
        FILE.write(
            "# 1:vza(pplane -ve = hs) 2:r(443) 3:r(565) 4:r(670) 5:r(765) 6:r(865)\n"
        )
        for i in range(len(vza)):
            ostr = str(vza[i]) + " "
            for j in range(5):
...
def invertData(
    ...
    phaseAngle = (
        numpy.arctan2(kk._Kernels__sinphaang, kk._Kernels__cosphaang) * 180.0 / numpy.pi
    )
...
def mimic(doPrint=False, doPlot=False, RossHS=False):
    ...
    vza = numpy.array(list(range(2 * r + 1))) * 1.0 - r
    # set up storage info
...
if __name__ == "__main__":
    ...
    doy, minrmse, minP, minFWD, minrefl, mindoy, sig, rel, fcc, a0, a1 = testLisa(
        inputFile
